    ...
    static async getHourlyEarnings(req, res, next) {
        try {
            ...
            const nonZeroHourlyEarnings = hourlyEarnings
                .map((earnings, hour) => ({
                    date: moment(transaksiToday.tanggalTransaksi).format('DD-MM-YYYY'),
                    hour,
                    earnings
                }))
                .filter(item => item.earnings > 0);

            console.log("transaksiToday.tanggalTransaksi:", transaksiToday.tanggalTransaksi);
            console.log("nonZeroHourlyEarnings:", nonZeroHourlyEarnings);

            // Format data response with date, start hour, end hour, and earnings
            const formattedData = nonZeroHourlyEarnings.map(item => ({
                date: transaksiToday.tanggalTransaksi,
