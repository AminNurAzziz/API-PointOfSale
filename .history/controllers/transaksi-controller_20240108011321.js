...
class TransaksiController {
    ...
    static async getHourlyEarnings(req, res, next) {
        try {
            ...
            const endOfTomorrow = moment().add(1, 'days').endOf('day'); // Include tomorrow
            ...
            const transaksiTodayAndTomorrow = await Transaksi.find({
                tanggalTransaksi: {
            ...
            for (let trans of transaksiTodayAndTomorrow) {
            ...
            // Filter out hours with zero earnings and include date information from each transaction
            const nonZeroHourlyEarnings = hourlyEarnings
                .map((earnings, hour) => ({
                    date: moment(transaksiTodayAndTomorrow[0].tanggalTransaksi).format('DD-MM-YYYY'), // Using the date from the first transaction
                    startHour: `${String(hour).padStart(2, '0')}:00:00`,
                    endHour: `${String(hour + 1).padStart(2, '0')}:00:00`,
                    earnings: earnings
                }))
