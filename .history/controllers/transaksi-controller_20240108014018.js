...
class TransaksiController {
    ...
    static async getHourlyEarnings(req, res, next) {
        try {
            ...
            const startOfDay = startDate ? moment(startDate).startOf('day').hour(15) : moment().startOf('day').hour(15);
            const endOfDay = endDate ? moment(endDate).endOf('day').hour(1) : moment().endOf('day').hour(1);
            ...
            for (let trans of transaksiToday) {
                ...
                // Filter transaksi berdasarkan jam (15:00 to 01:00)
                if ((hourIndex >= 15 && hourIndex <= 23) || (hourIndex >= 0 && hourIndex <= 1)) {
