...
class PendapatanController {
    ...
    static async addPendapatan(transaksi, listProduk, res) {
        try {
            // Mencari pendapatan berdasarkan tanggal tanpa jam
            const findPendapatan = await Pendapatan.findOne({ tanggalPendapatan: transaksi.tanggalTransaksi.toISOString().split('T')[0] });
            if (findPendapatan) {
                ...
                pendapatan.idAdmin = transaksi.idAdmin;
                pendapatan.totalPendapatan = transaksi.totalHarga;
                pendapatan.tanggalPendapatan = transaksi.tanggalTransaksi.toISOString().split('T')[0];
                listProduk.forEach((produk, index) => {
                    if (produk.kategoriProduk === 'Makanan') {
                        const totalPendapatanMakanan = produk.hargaProduk * transaksi.idProduk[index].jumlahProduk;
                ...
                console.log('Pendapatan berhasil ditambahkan');
            }
        } catch (err) {
            console.log(err);
        }
